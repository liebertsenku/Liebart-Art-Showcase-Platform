    ...
    <div class="py-12 bg-[#F5F4F2] min-h-screen">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                ...
                <h1 class="text-3xl font-bold text-gray-900 tracking-tight">Edit Challenge</h1>
                <p class="text-gray-500 mt-1">Update details for "{{ $challenge->title }}"</p>
            </div>

            <!-- Form Card -->
            <div class="bg-white rounded-[24px] shadow-sm p-8 border border-gray-100">
                <form action="{{ route('curator.challenges.update', $challenge->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4" required
                            class="w-full bg-[#F5F4F2] border-transparent focus:border-black focus:ring-0 rounded-xl px-4 py-3 text-gray-900 placeholder-gray-400 leading-relaxed">{{ old('description', $challenge->description) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="date" name="start_date" value="{{ old('start_date', $challenge->start_date->format('Y-m-d')) }}" required
                        ...
                        <div>
                            ...
                            <input type="date" name="end_date" value="{{ old('end_date', $challenge->end_date->format('Y-m-d')) }}" required
                                class="w-full bg-[#F5F4F2] border-transparent focus:border-black focus:ring-0 rounded-xl px-4 py-3 text-gray-900">
                    ...
                    <div>
                        ...
                        <textarea name="rules" rows="4" required
                            class="w-full bg-[#F5F4F2] border-transparent focus:border-black focus:ring-0 rounded-xl px-4 py-3 text-gray-900 placeholder-gray-400">{{ old('rules', $challenge->rules) }}</textarea>
                    ...
                    <div>
                        ...
                        <textarea name="prizes" rows="2"
                            class="w-full bg-[#F5F4F2] border-transparent focus:border-black focus:ring-0 rounded-xl px-4 py-3 text-gray-900 placeholder-gray-400">{{ old('prizes', $challenge->prizes) }}</textarea>
                    </div>

                    <!-- Banner Image -->
                    <div x-data="{ bannerPreview: '{{ $challenge->banner_image ? asset('storage/'.$challenge->banner_image) : '' }}' }">
                        ...
                        <div class="flex items-center justify-center w-full">
                            <label for="dropzone-file" class="relative flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-[#F5F4F2] hover:bg-gray-200 transition overflow-hidden">
                                ...
                                <div class="flex flex-col items-center justify-center pt-5 pb-6" x-show="!bannerPreview">
                                    ...
                                    <p class="text-sm text-gray-500"><span class="font-semibold text-black">Click to change</span> banner</p>
                                </div>

                                <img x-show="bannerPreview" :src="bannerPreview" class="absolute inset-0 w-full h-full object-cover">
                                
                                <div x-show="bannerPreview" class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 hover:opacity-100 transition duration-300">
                                    <span class="text-white font-bold bg-black/50 px-4 py-2 rounded-full backdrop-blur-sm">Change Image</span>
                                </div>
                    ...
                    <div class="flex items-center justify-end pt-6 border-t border-gray-100 gap-4">
                        <a href="{{ route('curator.challenges.index') }}" class="text-gray-500 font-bold hover:text-black">Cancel</a>
